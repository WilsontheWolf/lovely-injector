...
2. Open the .zip archive, copy `winmm.dll` into the game directory. You can navigate to the game's directory by right-clicking the game in Steam, hovering "Manage", and selecting "Browse local files".
...
5. **<ins>Only Steam Deck / Proton / Wine</ins>** Set your game's launch options in Steam to `WINEDLLOVERRIDES="winmm=n,b" %command%`.
...
4. Put one or more mods into the mod directory (NOT the same as the game directory). If you are modding Balatro, this should be `~/.local/share/love/Mods`.
5. Run the game by executing `./run_lovely_linux.sh` in the terminal within the game directory (you can copy this file into the directory of choice).

...
cargo +stage2 build --target aarch64-apple-ios $RELEASE --package lovely-mac

echo Building arm64e $TYPE build
cargo +stage2 build --target arm64e-apple-ios $RELEASE --package lovely-mac
...
if [ -z "$PACKAGE" ]; then
	...
	./crates/lovely-mac/build-deb.sh $TYPE universal
fi
